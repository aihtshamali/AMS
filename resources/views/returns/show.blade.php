...
@section('content')
    ...
    float: left;
    font-weight: bold ">Show Return</h3>
                                    ...
                                    <td style="background-color: #{{$item->color}};">
                                        @if($i<count($returndetails))
                                            @if($returndetails[$row][$col]==$item->id)
                                                <input type="number" class="<?=$i?>qty <?=$item->id?>item form-control" min="0"  name= "item[<?=$i?>][{{$item->id}}]" onchange="checkStock(<?=$i?>,a,{{$item->id}})" onblur="getTotal(this,<?=$i?>,{{$item->id}},{{Auth::user()->region->id}});"   value="{{$returndetails[$row][$col+1]}}">
                                                <?php $row++;?>
                                            @endif
                                        @else
                                            <input type="number" class="<?=$i?>qty <?=$item->id?>item form-control" min="0"  name= "item[<?=$i?>][{{$item->id}}]" onchange="checkStock(<?=$i?>,a,{{$item->id}})" onblur="getTotal(this,<?=$i?>,{{$item->id}},{{Auth::user()->region->id}});"   placeholder="{{$item->name}}">
                                        @endif
                                    </td>
                                @endfor
                                <?php $col++;$row=0;?>
